#!/usr/bin/env python3
"""Train a 20 Hz latent WM for DexMimicGen tactile rollouts.
    ...
    for path in paths:
        with h5py.File(path, "r") as f:
            ...
            for demo in sorted(root.keys(), key=demo_sort_key):
                low = root[demo]["low20"]
                length20 = int(low["action"].shape[0])
...
def compute_normalizers(
    ...
    for ref in refs:
        with h5py.File(ref.path, "r") as f:
            ...
            joint = pool_high200(demo["high200/robot_joint_pos"][:], joint_pool).reshape(-1, joint_dim)
            action = demo["low20/action"][:]
...
class ThreePiece20HzDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.latent_cache is not None:
            ...
            action = demo["low20/action"][start : start + self.window].astype(np.float32)
...
class TactileVAEProjector(nn.Module):
    ...
    def forward_latents(self, tactile_mu: torch.Tensor) -> torch.Tensor:
        # tactile_mu: (B, T, C, latent_dim), produced by the frozen VAE encoder.
...
class DexMGWM(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.tactile = TactileVAEProjector(
            vae_ckpt,
            channels=tactile_channels,
            embed_dim=embed_dim,
        )
        self.joint = MLPToken(joint_dim, embed_dim)
        self.fusion = CrossModalFusion(embed_dim=embed_dim, n_tokens=2)
        self.action = ActionEmbedder(action_dim, embed_dim)
    ...
    def forward(self, batch: dict[str, torch.Tensor], history: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pred = self.predictor(z[:, :history], a[:, :history])
    ...
    with h5py.File(tmp, "w") as out:
        for ref in refs:
            ...
            with h5py.File(ref.path, "r") as f:
                ...
                joint = pool_high200(joint_block, joint_pool).reshape(-1, joint_dim).astype(np.float32)
                action = demo["low20/action"][:].astype(np.float32)
                n = joint.shape[0]
                mu_all = np.empty((n, tactile_channels, 16), dtype=np.float32)
    ...
    meta.update(
        {
            ...
            "action_semantics": f"raw {args.action_dim}D low20 env delta action; no repeat-to-high200",
            "joint_dim": args.joint_dim,
            "action_dim": args.action_dim,
            "tactile_channels": args.tactile_channels,
