"""Collect DexMimicGen world-model rollouts with 20 Hz obs and high-rate tactile.
...
records high-rate samples at the requested target interval.
"""
...
POLICY_ACTION_TO_ENV_SUPPORTS_LAYOUT = (
    "action_layout" in inspect.signature(policy_action_to_env_action).parameters
)


def obs_keys_from_config(config) -> tuple[list[str], list[str]]:
    rgb_keys = []
    lowdim_keys = []
...
def convert_policy_action_to_env(
    action: np.ndarray,
    env_action_dim: int,
    rotation_transformer: RotationTransformer,
    action_layout: str,
) -> np.ndarray:
    if POLICY_ACTION_TO_ENV_SUPPORTS_LAYOUT:
        return policy_action_to_env_action(
            action, env_action_dim, rotation_transformer, action_layout
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--task-name",
        default=None,
        help="Optional task label stored in the output HDF5 attrs.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="High-rate tactile/joint/action sampling rate in Hz. Default keeps legacy high200.",
    )
...
def step_with_high_rate(env, action: np.ndarray, low_step_index: int, high_rate: int):
    if env.done:
    ...
    samples_per_control = int(round(control_dt * float(high_rate)))
    if samples_per_control <= 0:
        raise ValueError(
            f"high_rate={high_rate} gives no samples for control_dt={control_dt}"
        )
    sample_dt = 1.0 / float(high_rate)
    next_sample = 0
    samples = []
    ...
    for i in range(n_substeps):
        ...
        while next_sample < samples_per_control:
            ...
            samples.append(
                high_sample(env, action, target_time, actual_time, global_sim_step)
            )
            next_sample += 1
...
def collect_episode(
    ...
    rgb_keys, lowdim_keys = obs_keys_from_config(config)
    camera_names = camera_names_from_config(config)
    rotation_transformer = RotationTransformer(
    ...
    while low_steps < config.task.max_episode_steps:
        ...
        policy_action = dataset.normalizer["action"].unnormalize(
            act_normed.detach().cpu().numpy()
        )[0]
        start = config.task.obs_steps - 1
        end = start + config.task.act_steps
        act_env_seq = convert_policy_action_to_env(
        ...
        for act_env in act_env_seq:
            act_env = act_env.astype(np.float32)
            action_min = min(action_min, float(np.min(act_env)))
            action_max = max(action_max, float(np.max(act_env)))
            action_absmax = max(action_absmax, float(np.max(np.abs(act_env))))
            raw_obs, reward, done, _, samples = step_with_high_rate(
                env,
                act_env,
                low_steps,
                high_rate=high_rate,
...
def write_episode(
    ...
    group.attrs["tactile_hz"] = int(high_rate)
    group.attrs["high_rate_hz"] = int(high_rate)
    group.attrs["sim_hz"] = 500
    group.attrs["time_unit"] = "ms"

    low_group = group.create_group("low20")
    for key, arr in low.items():
        kwargs = image_kwargs if key.endswith("_image") and arr.ndim >= 4 else {}
        low_group.create_dataset(key, data=arr, **kwargs)

    high_group = group.create_group(f"high{int(high_rate)}")
...
def main():
    ...
    if 500 % int(args.high_rate) != 0:
        raise ValueError(
            f"--high-rate must divide the 500 Hz simulator rate cleanly, got {args.high_rate}"
        )
    ...
    print(f"[setup] high_rate={args.high_rate}Hz")
    print(
    ...
    env = make_env(args.env_dataset_path, config=config, enable_tactile=True)
    print(
    ...
    with h5py.File(out_path, "w") as h5:
        h5.attrs["task"] = args.task_name or Path(args.dataset_path).stem
        h5.attrs["format"] = f"wm_rollout_low20_high{int(args.high_rate)}"
        h5.attrs["time_unit"] = "ms"
        h5.attrs["control_hz"] = 20
        h5.attrs["tactile_hz"] = int(args.high_rate)
        h5.attrs["high_rate_hz"] = int(args.high_rate)
        h5.attrs["sim_hz"] = 500
