#!/usr/bin/env python3
"""Fit progress-aware contrastive setpoint sweeps for 20 Hz ThreePiece WM."""
...
def fit_binned(data, out: Path, k: int, sigma_scale: float, sigma_gap_fraction: float) -> dict:
    ...
    for b in range(k):
        ...
        aucs.append(binary_auc(np.concatenate([bp, bn]).tolist(), [1] * bp.size + [0] * bn.size))
    params = {
...
def fit_logistic(data, out: Path, basis: int, width: float, l2: float, steps: int, batch_size: int, device: torch.device, seed: int) -> dict:
    ...
    for step in range(steps):
        ...
        wb = phi @ b
        wz = torch.einsum("bm,md,bd->b", phi, w, zt[bi])
        logits = wz + wb
    ...
    auc = binary_auc(scores.tolist(), y[idx].astype(int).tolist())
    np.savez(
    ...
    return {"method": "B_progress_logistic", "basis": basis, "width": width, "l2": l2, "fit_auc_sample": float(auc), "steps": steps}
...
def fit_reftraj(data, out: Path, k: int, sigma_scale: float) -> dict:
    ...
    return {
        ...
        "k": k,
        "sigma_scale": sigma_scale,
        "late_auc": binary_auc(np.concatenate([beta_pos, beta_neg]).tolist(), [1] * beta_pos.size + [0] * beta_neg.size),
    }
...
def main() -> None:
    ...
    train_success, _ = split_groups(success, args.val_demos_per_class)
    train_failure, _ = split_groups(failure, args.val_demos_per_class)
    ...
    rows = []
    for kind, name, cfg in configs:
        out = out_root / name
        out.mkdir(parents=True, exist_ok=True)
        if kind == "A":
            metrics = fit_binned(data, out, cfg["k"], cfg["sigma_scale"], 0.25)
        elif kind == "B":
            metrics = fit_logistic(data, out, cfg["basis"], cfg["width"], cfg["l2"], 1200, 8192, device, args.seed + len(rows))
        else:
            metrics = fit_reftraj(data, out, cfg["k"], cfg["sigma_scale"])
        meta = {"name": name, "kind": kind, **metrics}
