#!/usr/bin/env python3
"""Train ThreePiece 20 Hz WM directly from a latent-cache HDF5.

The input cache is flat groups with:

- tactile_mu: (T, 4, 16)
- joint: (T, 14), raw robot joint position
- action: (T, 14), raw low20 env action
...
from train_threepiece_20hz_wm import SIGReg, ThreePieceWM, batch_to_device
...
def compute_normalizers(cache_path: str, groups: list[str]) -> dict[str, np.ndarray]:
...
@torch.no_grad()
def evaluate(model: ThreePieceWM, loader: DataLoader, sigreg: SIGReg, device: torch.device, args) -> dict[str, float]:
    model.eval()
...
def main() -> None:
    ...
    norms = compute_normalizers(args.cache, train_groups)
    ...
    meta.update(
        {
            ...
            "action_semantics": "raw 14D low20 env delta action, normalized from cache",
