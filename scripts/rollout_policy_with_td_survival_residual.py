#!/usr/bin/env python3
"""Roll out a tactile policy with a 20 Hz TD/survival value residual action."""
...
from train_threepiece_td_survival_value import SurvivalQHead  # noqa: E402
...
def pad_obs_slice(arr: np.ndarray, start: int, length: int) -> np.ndarray:
    n = int(arr.shape[0])
    if n == 0:
        raise RuntimeError("cannot pad an empty observation array")
    idx = np.arange(start, start + length)
    idx = np.clip(idx, 0, n - 1)
    return arr[idx]


def pad_action_slice(arr: np.ndarray, start: int, length: int) -> np.ndarray:
    n = int(arr.shape[0])
    if n == 0:
        return np.zeros((length, 14), dtype=np.float32)
    idx = np.arange(start, start + length)
    idx = np.clip(idx, 0, n - 1)
    return arr[idx]
...
def value_logit_20hz(
    wm: ThreePieceWM,
    value_head: SurvivalQHead,
    tactile_hist_np: np.ndarray,
    joint_hist_np: np.ndarray,
    action_past_np: np.ndarray,
    ...
    action_past = torch.from_numpy(action_past_np.astype(np.float32)).to(device=device, dtype=torch.float32)
    action_full = torch.cat([action_past, future_low_raw], dim=0)[None]
    action_norm = normalize_action(action_full, normalizers, device)
    if action_norm.shape[1] != history - 1 + future_low_raw.shape[0]:
        raise RuntimeError(f"bad action context shape {action_norm.shape}")
    ...
    return value_head(z_hist, action_chunk, z_window[:, -1]).squeeze(0)
...
def optimize_residual_20hz(
    ...
    with torch.no_grad():
        residual_logit = value_logit_20hz(
            wm,
            value_head,
            tactile_hist_np,
            joint_hist_np,
    ...
    return {
        "delta0": delta0,
        "grad0": grad0,
        "value_base": float(torch.sigmoid(base_logit).detach().cpu()),
        "value_residual": float(torch.sigmoid(residual_logit).detach().cpu()),
...
def score_q_values_20hz(
    ...
    for low_start in range(0, n_low, batch_size):
        ...
        action_batch = []
        for low_idx in lows:
            obs_start = low_idx - history + 1
            tactile_batch.append(pad_obs_slice(tactile_low, obs_start, history))
            joint_batch.append(pad_obs_slice(joint_low, obs_start, history))
            action_start = low_idx - history + 1
            action_batch.append(pad_action_slice(action_low, action_start, history - 1 + chunk))
        ...
        for k in range(chunk):
            ...
            next_z = pred_seq[:, -1]
            z_window = torch.cat([z_window[:, 1:], next_z[:, None]], dim=1)
        action_chunk = sample["action"][:, history - 1 : history - 1 + chunk]
        logits = value_head(z_hist, action_chunk, z_window[:, -1])
        values.extend(torch.sigmoid(logits).detach().cpu().numpy().astype(np.float32).tolist())
    return np.asarray(values, dtype=np.float32)


def collect_one_residual_rollout(env, config, dataset, agent, wm, value_head, normalizers, nfe: int, episode_idx: int, args):
        ...
        for j, base_act14 in enumerate(act14_seq):
            ...
            if args.progress_every > 0 and low_steps % args.progress_every == 0:
                print(
                    ...
                    f"value_base={opt['value_base']:.3f} value_res={opt['value_residual']:.3f} "
...
def save_q_plot(episodes: list[dict], output: Path) -> None:
    ...
    ax.set_title("20 Hz TD/survival residual policy value score")
    ax.set_xlabel("time (s)")
    ax.set_ylabel("survival value = sigmoid(logit)")
...
def parse_args():
    ...
    parser.add_argument(
        "--value-ckpt",
        default=str(DEXMG_ROOT / "outputs/td_survival_value_experiments/heads/td_gamma995/q_best.pt"),
    )
    parser.add_argument(
        "--wm-ckpt",
    ...
    parser.add_argument("--out-dir", default=str(DEXMG_ROOT / "outputs/td_survival_value_experiments/evals/td_gamma995_trust005_reg20"))
...
def main() -> None:
    ...
    print(f"[setup] value_ckpt={args.value_ckpt}", flush=True)
    ...
    value_head = SurvivalQHead(history=args.history, chunk=args.chunk).to(device)
    value_ckpt = torch.load(args.value_ckpt, map_location=device)
    value_cfg = value_ckpt.get("config", {})
    if int(value_cfg.get("history", args.history)) != args.history or int(value_cfg.get("chunk", args.chunk)) != args.chunk:
        raise RuntimeError(f"value checkpoint history/chunk mismatch: {value_cfg}")
    value_head.load_state_dict(value_ckpt["q_head"], strict=True)
    value_head.eval()
    ...
    for ep in range(args.episodes):
        ...
        data = collect_one_residual_rollout(env, config, dataset, agent, wm, value_head, normalizers, args.nfe, ep, args)
        value_scores = score_q_values_20hz(
            wm,
            value_head,
            data["high_tactile"],
            data["high_joint"],
        ...
        value_base = [float(r["value_base"]) for r in data["residual"]]
        value_res = [float(r["value_residual"]) for r in data["residual"]]
        clip_fracs = [float(r["clip_frac"]) for r in data["residual"]]
        ep_meta = {
            "episode": ep,
            "success": bool(data["success"]),
            "reward_sum": float(data["reward_sum"]),
            "steps": int(data["steps"]),
            "q_min": float(value_scores.min()) if value_scores.size else float("nan"),
            "q_max": float(value_scores.max()) if value_scores.size else float("nan"),
            "q_mean": float(value_scores.mean()) if value_scores.size else float("nan"),
            "q_first": float(value_scores[0]) if value_scores.size else float("nan"),
            "q_last": float(value_scores[-1]) if value_scores.size else float("nan"),
            "q_base_mean": float(np.mean(value_base)) if value_base else float("nan"),
            "q_residual_mean": float(np.mean(value_res)) if value_res else float("nan"),
            ...
            "clip_frac_mean": float(np.mean(clip_fracs)) if clip_fracs else float("nan"),
            "q_values": value_scores.astype(float).tolist(),
    ...
    summary = {
        ...
        "q_timebase": (
            "one survival value score per 20Hz low-control step; tactile/joint pooled "
            "with last high200 sample; action context is history-1 past plus chunk current/future actions"
        ),
        "episodes": episodes,
    }
